...
@app.route('/')
def index():
	return render_template('Ravi/Index.html')

@app.route('/login', methods = ['GET', 'POST'])
def login():
	if request.method == 'POST':
		username = request.form['username']
		password_candidate = request.form['password']     # form 
		cur = mysql.connection.cursor()  #This is equal to st of java

		result = cur.execute('SELECT * FROM info WHERE username = %s', [username])
		#print(result)
		if result>0:
			data = cur.fetchone()    # fetches a row from database
